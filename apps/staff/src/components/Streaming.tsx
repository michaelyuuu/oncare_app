...
export function Streaming({ visits, onAction, pending, errors }: {
    ...
    return <ul className="rows">{visits.map(v => {
        ...
        const cameraError = errors[`${base}/camera`];
        const endError = errors[`${base}/end`];
        return <li key={v.id} className="call-row">
            ...
            <div className="actions">
                {((v.cameraState === "on" || v.cameraState === "paused") || cameraError) && <div className="call-action">
                    ...
                    {cameraError && <p className="row-error" role="alert">{cameraError}</p>}
                </div>}
                <div className="call-action">
                    <button disabled={ending} onClick={() => void onAction(`${base}/end`)}>{t("staff.streaming.end")}</button>
                    {endError && <p className="row-error" role="alert">{endError}</p>}
                </div>
            </div>
        </li>;
    })}</ul>;
