...
The unified entrance is available at `http://localhost:5172`; it links to `http://localhost:5173` (resident kiosk), `http://localhost:5174` (family), and `http://localhost:5175` (staff and manager sign-in). Choosing a role only navigates to an application: it does not authenticate the user or grant authority. In particular, the Manager link adds the presentation hint `mode=manager`, while the staff app grants Facility admin access only when `/auth/login` returns an authenticated admin principal.

Start the synthetic gateway separately from `robot_gateway/` with `ONCARE_ROBOT_TOKEN=robot-demo-token` and `ROBOT_ADAPTER=mock`; it must log `SIMULATED ROBOT`. Demo-only identity or provider affordances must be explicitly enabled and visibly labelled `SIMULATED` (the browser acceptance configuration sets `VITE_ONCARE_DEMO=1`). Production deployments must leave synthetic identity selection disabled and must never treat a client-supplied role, query parameter, or synthetic actor header as authentication. Copy `apps/api/.env.example` to `apps/api/.env` when configuring a real LiveKit provider. Never commit real credentials.
