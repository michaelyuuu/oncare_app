...
2. While the call is connecting or active, the family phone can open **Ask the robot for help**. For the clearest handover sequence, end the real call first and then open **Ask the robot for help** on the completed visit. The delivery notice has lower display precedence than an incoming or active call: if a delivery arrives while the call is still shown, the resident iPad continues to show the call; the delivery notice appears after the call ends.
...
4. Click **Yes, send the robot**. The family phone advances to **Care home approval**. The staff console gains a pending task with **Approve** and **Deny**. The gateway log still must not show a `deliver_item` intent.
...
6. When the staff console offers **Loaded on tray**, narrate that staff would load the tray, but perform no physical action; click **Loaded on tray** to advance the software-only demo. The gateway log reports the delivery leg and arrival. The family phone advances toward delivery.
...
2. On the staff console, click the large red **STOP ROBOT** button. The family phone shows **The robot was stopped for safety** and the task audit ends in `safety_stopped` with reason `staff_stop`. STOP does not auto-resume.
...
1. On the family phone, open **Ask the robot for help**, type `bring her medication`, and click **Send**.
