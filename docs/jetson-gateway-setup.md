...
## Attended rehearsal checklist — NOT RUN

Preparation only: every box is unchecked; there are no physical success or latency results. Execute only after separate owner authorization, successful attended doctor/preflight above, a clear corridor, and an attendant beside the robot with the nav_web STOP page open on a phone. The owner must be present. Startup, manual driving, network interruption, location edits and recovery require that gate; this document does not authorize them. Stop on unexpected motion or failed prerequisites.

- [ ] **Record three approved positions.** The owner manually drives with nav_web, not OnCare, to standby, pickup station and demo room. At each stationary position select the matching staff row and **Use robot's position here**: this immediately PATCHes (no second Save). Capture requires connection and a valid heartbeat no older than six seconds. Manual coordinate edits instead require **Save**. Verify returned approved coordinates and the full gateway locations update using actual API/WebSocket evidence; do not assume a dedicated `locations received` log exists. Back up all three rows exactly and verify the map and route.
- [ ] **Visit.** Submit the family visit request and staff approval. Observe acceptance, `robot_en_route`, then `awaiting_resident_consent` after matching Nav2 success. Check the resident iPad rings; answer, verify video/audio with consent, and end the call. Record request, departure, Nav2 arrival and ring timestamps; calculate request-to-ring latency and Nav2 travel time. Add observed results to a **Real robot** section of [demo-visit-flow.md](demo-visit-flow.md), retaining its mock instructions.
- [ ] **Tray delivery.** Request “bring the water bottle”, confirm and approve; observe pickup arrival, human tray loading, **Loaded on tray**, room arrival, resident **I have it**, then approved standby arrival. Record each transition time in a **Real robot** section of [demo-task-flow.md](demo-task-flow.md). No onward leg may start before its human acknowledgment. Rest-lift is allowed only after successful approved standby arrival with no active navigation; queued is not proof of physical lift completion.
- [ ] **STOP / PIN / standby drill.** During an owner-approved drive press staff STOP. Observe physical halt separately from software state: soft e-stop stays latched, task becomes `safety_stopped`, family sees the message. Measure click-to-halt rather than assuming a guarantee. After the owner confirms the route clear, re-enter the PIN to release; verify **Return to standby** only when ready and not busy. Ordinary cancel resumes; STOP must not automatically resume.
- [ ] **Network-loss drill.** With explicit owner approval during an attended drive, disconnect the Jetson network for 15 seconds. Observe `link down`, physical halt and the expected safety transition after the 10-second disconnect grace. Record actual disconnect, detection, halt and reconnect times. Reconnect and inspect the staff `safety_stopped` event; preserve audit/state evidence instead of assuming a dedicated gateway `safety_stopped` log. Cloud disconnection is not loss of the local nav_web link or a guaranteed physical stopping bound.
- [ ] **Occupied-cell rejection drill.** With no active task/navigation, back up an isolated test destination exactly. The owner selects a known occupied interior cell on the current map, avoiding an outside/unknown-cell rejection instead. Authorized staff PATCH that destination temporarily and request the test visit. Require `navigation_failed` with reason `occupied` and evidence that no `/goal` was sent for it. Never send a raw goal or bypass map validation. Restore and verify the original row before any further task; on unexpected navigation, STOP and abandon the drill.
- [ ] **Write actual results only after execution.** Create `docs/rehearsal-YYYY-MM-DD.md` using the real run date: owner/attendant sign-off, software/map versions, pass/fail/not-run per step, measured times and clock basis, redacted evidence paths, manual interventions and unresolved issues. Link the two updated demo documents. Do not replace synthetic fixtures or remove markers without separately authorized genuine captures and provenance. The fixed-name health renderer still blocks an `oncare_gateway` tile; UDP emission alone is not tile success. Include the preceding HTTP/STOP limitations alongside observations.

### Source contract to verify against the deployed revision

The current [adapter parser](../robot_gateway/gateway/robot/navweb.py) reads these keys; these are source-derived, not verified live captures:

| Response | Exact keys and interpretation |
| --- | --- |
| `/state` | `pose`: finite `[x, y, yaw]`; `estop`: strict boolean (missing/invalid fails closed). |
| `/state` stack | `stack.running` must be `true`; `stack.mode` must be `nav` or `explore`. |
| `/state` navigation | `nav.state`: `idle`; `sending`/`navigating` → active; `succeeded`; `aborted`/`rejected`/`unavailable` → failed; `canceled`; other/missing → unknown. `nav.dist`: finite nonnegative distance. `nav.goal`: finite `[x, y, yaw]`, matched to target rounded to three decimal places for x/y and four for yaw before accepting success. |
| `/state` lift | `lift.busy`, falling back to `lift.last`, otherwise unknown. No battery field is inferred. |
| `/health.json` | `tiles[]`: a tile with `key == "base_rpc"`, `state == "ok"`, `value == "up"` is required for base readiness. |

Readiness also requires fresh readable snapshots, valid pose/navigation state and no local safety latch. During source automatic recovery, `canceled` without e-stop remains pending until progress or bounded no-progress timeout, not immediate local completion. Record any live schema mismatch as failure, not permission to bypass readiness or edit the sibling robot stack.
